...
function addNotification(type, course_id, source_id, timestamp, ref_id){
	client.query("INSERT INTO `notifications` (`type`, `course_id`, `source_id`, `timestamp`, `reference`) VALUES (?, ?, ?, ?, ?)", [type, course_id, source_id, timestamp, ref_id], function(err, result){
		if(err) throw err;
	});
}
...
exports.coursepage = function(req, res){
	...
	if(p == "main" || p == ""){
		client.query("SELECT * FROM `posts` INNER JOIN `user` ON `author_id` = `user_id` WHERE `course_id` = ? ORDER BY `post_timestamp` DESC", [course_det.course_id], function(err, rows, fields){
		if(err) throw err;
		...
		if(r == 0){
			client.query("select * from `assignments` left join (select * from `submissions` where `user_id` = ?) as t on `id` = `assignment_id` where `course_id` = ?", [req.session.user.user_id, course_det.course_id], function(err, rows, fields){
				if(err) throw err;
			...
			client.query("SELECT * FROM `assignments`  WHERE `course_id` = ? ORDER BY `creation_date` ASC", [course_det.course_id], function(err, rows, fields){
				if(err) throw err;
				res.render('assgnpage_instr', {loc: course_det, data: req.session.courses, portal: p, me: req.session.user, assgns: rows, role: r, notif: req.notifications});
			});
		}
		...
		client.query("SELECT * FROM `user_course` INNER JOIN `user` ON `user_course`.`user_id` = `user`.`user_id` WHERE `course_id` = ? ORDER BY `role` DESC", [course_det.course_id], function(err, rows, fields){
			if(err) throw err;
		...
		client.query("SELECT * FROM `resources` INNER JOIN `user` ON `resources`.`user_id` = `user`.`user_id` WHERE `course_id` = ? ORDER BY `upload_date` DESC", [course_det.course_id], function(err, res_rows, fields){
			if(err) throw err;
			client.query("SELECT * FROM `resource_links` INNER JOIN `user` ON `resource_links`.`user_id` = `user`.`user_id` WHERE `course_id` = ? ORDER BY `upload_date` DESC", [course_det.course_id], function(err, link_rows, fields){
				if(err) throw err;
...
exports.addcourse = function(req, res){
	...
	if(!name || !clab || !ckey || !name.trim() || !clab.trim() || !ckey.trim())
		...
		client.query("INSERT INTO `courses` (`course_label`, `course_name`, `course_desc`, `course_key`) VALUES ( ?, ?, ?, ? )", [clab, name, desc, ckey], function(err, results){
			if(err) throw err;
			var id = results.insertId;
			client.query("INSERT INTO `user_course` (`user_id`, `course_id`, `role`) VALUES (?, ?, 1)", [req.session.user.user_id, id], function(err, result){
				if(err) throw err;
...
exports.userpage = function(req, res){
	...
	portal = portal || "";
	client.query("SELECT * FROM `user` WHERE `user_id` = ?", [user_id], function(err, rows, fields){
		if(err) throw err;
		var userdet = rows[0];
...
exports.assignment_det = function(req, res){
	...
	if(req.coursedet.role != 1)
		...
		client.query("select first_name, last_name, T.user_id, img_id, submission_id, submission_date, filename from (select `user`.* from `user_course` inner join `user` on `user_course`.`user_id` = `user`.`user_id` where course_id = ? and role = 0) as T left join `submissions` on T.`user_id` = `submissions`.`user_id` and (`assignment_id` = ? or `assignment_id` is null)", [course_id, a_id], function(err, rows, fields){
			if(err) throw err;
			client.query("SELECT * FROM `assignments` WHERE `id` = ?", [a_id], function(err, assg, fields){
				if(err) throw err;
				if(assg[0])
...
exports.comments = function(req, res){
	...
	if(req.session.user.user_id == uid || req.coursedet.role == 1){
		client.query("SELECT * FROM `a_comments`INNER JOIN `user` ON `author_id` = `user_id` WHERE `assignment_source` = ? AND `assignment_id` = ?", [uid, aid], function(err, rows, fields){
			if(err) throw err;
...
exports.editprofile = function(req, res){
	...
	if(file_upload){
		client.query("UPDATE `user` SET `first_name` = ?, `last_name` = ?, `email_id` = ?, `img_id` = ? WHERE `user_id` = ?", [fn, ln, emid, name, req.session.user.user_id], function(err, results){
			if(err) throw err;
		});
	}
	else{
		client.query("UPDATE `user` SET `first_name` = ?, `last_name` = ?, `email_id` = ? WHERE `user_id` = ?", [fn, ln, emid, req.session.user.user_id], function(err, results){
			if(err) throw err;
		});
	}
	res.redirect("/user/profile/");
...
exports.extraoptions = function(req, res){
	...
	{
		var id = req.body.id;
		client.query("SELECT * FROM `assignments` WHERE `id` = ?", [id], function(err, rows, fields){
			if(err) throw err;
			if(rows[0])
	...
	else if(option == "file_upload" && portal == "resources")
	{
		...
		if(file.filename && file.size && file.name){
			...
			fs.rename(tmp_path, target_path, function(err){if(err) throw err;});
			client.query("INSERT INTO `resources` (`course_id`, `upload_date`, `filename`, `explanation`, `filetype`, `user_id`) VALUES (?, ?, ?, ?, ?, ?)", [req.session.current_course, new Date().getTime() / 1000, file.name, file_expln, file.type, req.session.user.user_id], function(err, result){
				if(err) throw err;
		...
		if(vid){
			...
			client.query("INSERT INTO `resource_links` (`course_id`, `upload_date`, `link_name`, `explanation`, `link_url`, `user_id`, `link_type`) VALUES (?, ?, ?, ?, ?, ?, ?)", [req.session.current_course, new Date().getTime() / 1000, "_", vid_expln, vid, req.session.user.user_id, "VIDEO_EMBED"], function(err, result){
				if(err) throw err;
				addNotification('RESOURCE', req.params.course_id, req.session.user.user_id, new Date().getTime() / 1000, result.insertId);
			});
		}
		...
		if(link_url){
			...
			client.query("INSERT INTO `resource_links` (`course_id`, `upload_date`, `link_name`, `explanation`, `link_url`, `user_id`, `link_type`) VALUES (?, ?, ?, ?, ?, ?, ?)", [req.session.current_course, new Date().getTime() / 1000, link_name, link_expln, link_url, req.session.user.user_id, "HYPERLINK"], function(err, result){
				if(err) throw err;
				addNotification('RESOURCE', req.params.course_id, req.session.user.user_id, new Date().getTime() / 1000, result.insertId);
	...
	else if(option == "assignment_submission" && portal == "assignments")
	{
		...
		if(!file.size || !file.name || !file.filename){
			...
			fs.rename(tmp_path, target_path, function(err){if(err) throw err;});
			client.query("INSERT INTO `submissions` (`submission_id`, `assignment_id`, `submission_date`, `user_id`, `filename`) VALUES (?, ?, ?, ?, ?)", [fileid, a_id, date, req.session.user.user_id, name], function(err, result){
				if(err) throw err;
			});
			res.redirect("/courses/" + req.params.course_id + "/assignments/");
		}
...
exports.searchpage = function(req, res){
	...
	client.query(cq, function(err, rows, fields){
		if(err) throw err;
		course_results = rows;
		client.query(uq, function(err, urows, fields){
			if(err) throw err;
			client.end();
...
exports.person_interac = function(req, res){
	...
	if(req.coursedet.role != 1){
		...
		client.query("SELECT * FROM `user` INNER JOIN `user_course` ON `user`.`user_id` = `user_course`.`user_id` WHERE `user`.`user_id` = ? AND `course_id` = ? AND `role` = 0", [pid, cid], function(err, rows, fields){
			if(err) throw err;
			if(!rows){
				res.send("An error occurred! The specified student is not registered for this course!");
			}
			else{
				client.query("select * from `assignments` left join `submissions` on `assignments`.`id` = `submissions`.`assignment_id` and `user_id` = ? where course_id = ?", [pid, cid], function(err, det, fields){
...
exports.joincourse = function(req, res){
	...
	client.query("SELECT * FROM `user_course` INNER JOIN `courses` ON `courses`.`course_id` = `user_course`.`course_id` WHERE `user_id` = ? AND `course_key` = ?", [req.session.user.user_id, ckey], function(err, rows, fields){
		if(err) throw err;
		if(rows.length){res.send("-1"); return;}//already a member of the course
		client.query("SELECT * FROM `courses` WHERE `course_key` = ?", [ckey], function(err, rows, fields){
			if(err) throw err;
			if(!rows.length){res.send("0"); return;} //no course with this key
			else{
				var id = rows[0].course_key;
				req.session.courses.push(rows[0]);
				client.query("INSERT INTO `user_course` (`user_id`, `course_id`, `role`) VALUES (?, ?, 0)", [req.session.user.user_id, rows[0].course_id, 1], function(err, result){if(err) throw err; res.send(JSON.stringify(rows[0])); return;});
			}
		});
	});
}
...
exports.course_edit = function(req, res){
	...
	if(file_upload){
		client.query("UPDATE `courses` SET `course_label` = ?, `course_name` = ?, `masthead_img` = ?, `course_key` = ? WHERE `course_id` = ?", [clab, cname, name, ckey, cid], function(err, rows, fields){if(err) throw err; });
	}else{
		client.query("UPDATE `courses` SET `course_label` = ?, `course_name` = ?, `course_key` = ? WHERE `course_id` = ?", [clab, cname, ckey, cid], function(err, rows, fields){if(err) throw err; });
	}
